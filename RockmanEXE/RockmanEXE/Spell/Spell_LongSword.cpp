...
void CSpell_LongSword::Initialize(void)
{
	CSpell::Initialize_Spell();

	// 현재 좌표 박스로 조정하기
	m_vecBox = CVector3<float>(36.f, 8.f, 8.f);
	m_vecBoxPos = CVector3<float>((float)m_vecDirection.x * 18.f, 0.f, 0.f);
	m_vecSpeed = CVector3<float>(0.f, 0.f, 0.f);

#pragma region 이미지 추가
	wstring sText = L"./RockmanEXE/Resource/battle/navi/spell/sword/";
	CBmpMgr::Get_Instance()->Insert_PNG((sText + L"long_sword.png").c_str(), L"NBT_Spell_Long_Sword");

	Set_FrameKey(0, L"NBT_Spell_Long_Sword");
...
int CSpell_LongSword::Update(float fDeltaTime)
{
...
void CSpell_LongSword::Late_Update(float fDeltaTime)
{
	// 충돌 처리
	if (!m_bHit)
	{
...
void CSpell_LongSword::Render(HDC hDC)
{
	CBmpMgr::Get_Instance()->Draw_PNG_Strip(hDC, Get_FrameKey(0), Get_Frame(0), m_vecPos, m_vecDirection);
}

void CSpell_LongSword::Release(void)
{

}

void CSpell_LongSword::Collide(CObj* _pDst)
{
	// 공격용 유닛은 대미지를 준다.
